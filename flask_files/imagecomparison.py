...
corpus_metadata = pd.read_json('./static/models/corpus_metadata_final.json')
...
def compare_test_image(test_image_file, metric='cosine'):
    ...
    distancelist = distance_array.argsort()[0][0+shift:25+shift].tolist()
    print(distancelist)
    ...
    for index, value in enumerate(top5ids):
        artwork = {}
        artwork['order'] = index
        artwork['image_url'] = corpus_metadata.loc[corpus_metadata['id'] == int(top5ids[index]), 'image_url'].item()
        artwork['artist'] = corpus_metadata.loc[corpus_metadata['id'] == int(top5ids[index]), 'artist'].item()
        artwork['title'] = corpus_metadata.loc[corpus_metadata['id'] == int(top5ids[index]), 'title'].item()
        artwork['date'] = corpus_metadata.loc[corpus_metadata['id'] == int(top5ids[index]), 'date'].item()
        artwork['medium'] = corpus_metadata.loc[corpus_metadata['id'] == int(top5ids[index]), 'medium'].item()
        artwork['page_url'] = corpus_metadata.loc[corpus_metadata['id'] == int(top5ids[index]), 'page_url'].item()
